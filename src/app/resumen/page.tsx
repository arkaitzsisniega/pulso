    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-4">
      ...
      {tab === "general" && (
        <div className="space-y-4">
          ...
          <div className="bg-zinc-900 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="bg-red-900/30 rounded-lg p-5">
                ...
                {/* De los disparos en contra, cuántos salieron de una
                    incorporación del portero rival. Va DENTRO del cuadro de
                    disparos y en una línea, para poder leerlo de un vistazo al
                    acabar la parte o el partido (Arkaitz 22/8/2026). */}
                {(partido.incorporacionesRival?.conDisparo ?? 0) > 0 && (
                  <div className="mt-3 pt-2 border-t border-red-700/30
                                  text-sm text-red-300/90">
                    🧤 {t("res_disparos_de_incorporacion", {
                      n: partido.incorporacionesRival!.conDisparo,
                      total: partido.disparosRival.puerta + partido.disparosRival.palo
                             + partido.disparosRival.fuera + partido.disparosRival.bloqueado,
                    })}
                    <span className="text-red-400/70">
                      {" "}({t("res_incorporaciones_total", {
                        n: partido.incorporacionesRival!.total,
                      })})
                    </span>
                  </div>
                )}
          ...
          {(() => {
            // La anticipación y el pase de gol los tienen todos, porteros
            // incluidos: los del portero van en SU tabla y no también en esta.
            const conDatosCampo = filasIndiv.filter((f) => f.c && (
              ...
              || (!f.esPortero && (f.c.antB||0)+(f.c.antM||0)+(f.c.paseGol||0) > 0)
            ));
                    ...
                    <table className="w-full text-base">
                      <thead className="text-xs text-zinc-400 border-b border-zinc-800">
                        <tr>
                          ...
                          <th className="px-2" title={t("vid_pase_gol")}>{t("inf_pase_gol")}</th>
                      ...
                      <tbody>
                        {conDatosCampo.map((f) => (
                          <tr key={f.nombre} className="border-b border-zinc-900 text-center">
                            ...
                            <td className="px-2 font-mono">{f.esPortero ? "—" : (f.c?.paseGol||0)}</td>
                      ...
                      <tbody>
                        {conDatosPort.map((f) => (
                          <tr key={f.nombre} className="border-b border-zinc-900 text-center">
                            ...
                            <td className="px-2 font-mono">{f.c?.paseGol||0}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
